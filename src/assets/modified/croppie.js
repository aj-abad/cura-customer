...
(function (root, factory) {
  ...
  var emptyStyles = typeof document !== 'undefined' ? document.createElement('div').style : {}
  ...
  function getExifOffset (ornt, rotate) {
    ...
    var index = arr.indexOf(ornt)
    var offset = (rotate / 90) % arr.length// 180 = 2%4 = 2 shift exif by 2 indexes
  ...
  Transform.fromString = function (v) {
    var values = v.split(') ')
    var translate = values[0].substring(Croppie.globals.translate.length + 1).split(',')
  ...
  function _create () {
    ...
    var customViewportClass = self.options.viewport.type ? 'cr-vp-' + self.options.viewport.type : null
    var boundary; var img; var viewport; var overlay; var bw; var bh
  ...
  function _initializeZoom () {
    var self = this
    var wrap = self.elements.zoomerWrap = document.createElement('div')
    var zoomer = self.elements.zoomer = document.createElement('input')
  ...
  function _onZoom (ui) {
    var self = this
    var transform = ui ? ui.transform : Transform.parse(self.elements.preview)
    var vpRect = ui ? ui.viewportRect : self.elements.viewport.getBoundingClientRect()
    var origin = ui ? ui.origin : new TransformOrigin(self.elements.preview)
  ...
  function _getVirtualBoundaries (viewport) {
    ...
    var centerFromBoundaryX = self.elements.boundary.clientWidth / 2
    var centerFromBoundaryY = self.elements.boundary.clientHeight / 2
  ...
  function _updateCenterPoint (rotate) {
    var self = this
    var scale = self._currentZoom
    var data = self.elements.preview.getBoundingClientRect()
    var vpData = self.elements.viewport.getBoundingClientRect()
    var transform = Transform.parse(self.elements.preview.style[CSS_TRANSFORM])
    var pc = new TransformOrigin(self.elements.preview)
  ...
  function _updateZoomLimits (initial) {
    var self = this
    var minZoom = Math.max(self.options.minZoom, 0) || 0
    var maxZoom = self.options.maxZoom || 5.0
    var initialZoom
    var defaultInitialZoom
    var zoomer = self.elements.zoomer
    var scale = parseFloat(zoomer.value)
    var boundaryData = self.elements.boundary.getBoundingClientRect()
    var imgData = naturalImageDimensions(self.elements.img, self.data.orientation)
    var vpData = self.elements.viewport.getBoundingClientRect()

    const bound = Math.min(imgData.height, imgData.width)
    minZoom = this.options.viewport.height / bound
    // if (self.options.enforceBoundary) {
  ...
  function _centerImage () {
    var self = this
    var imgDim = self.elements.preview.getBoundingClientRect()
    var vpDim = self.elements.viewport.getBoundingClientRect()
    var boundDim = self.elements.boundary.getBoundingClientRect()
  ...
  function _getCanvas (data) {
    ...
    var left = num(points[0])
    var top = num(points[1])
    var right = num(points[2])
    var bottom = num(points[3])
    ...
    var startX = 0
    var startY = 0
    var canvasWidth = data.outputWidth || width
    var canvasHeight = data.outputHeight || height
  ...
  function _bind (options, cb) {
    ...
    var points = []
    var zoom = null
    var hasExif = _hasExif.call(self)
  ...
  function _get () {
    ...
    var widthDiff = (vpData.width - self.elements.viewport.offsetWidth) / 2 // border
    var heightDiff = (vpData.height - self.elements.viewport.offsetHeight) / 2
    var x2 = x1 + self.elements.viewport.offsetWidth + widthDiff
    var y2 = y1 + self.elements.viewport.offsetHeight + heightDiff
    var scale = self._currentZoom
  ...
  function _result (options) {
    var self = this
    var data = _get.call(self)
    var opts = deepExtend(clone(RESULT_DEFAULTS), clone(options))
    var resultType = (typeof (options) === 'string' ? options : (opts.type || 'base64'))
    var size = opts.size || 'viewport'
    var format = opts.format
    var quality = opts.quality
    var backgroundColor = opts.backgroundColor
    var circle = typeof opts.circle === 'boolean' ? opts.circle : (self.options.viewport.type === 'circle')
    var vpRect = self.elements.viewport.getBoundingClientRect()
